...
* Using `/slim` and non `/slim` imports in the same codebase no longer throws
  exceptions about failed `instanceof` checks when handling values wrapped in
  classes generated by `wasm-bindgen`.

### Changed

* ESM bundler entrypoints load the standalone Wasm asset through
  `new URL(..., import.meta.url)` and top-level await in both development and
  production. For Vite 8 this means no wasm or top-level await plugins are
  required.
...
* Vite 8 or newer is required for zero-configuration Vite development. Vite 7
  and older projects must list the generated Wasm package in
  `optimizeDeps.exclude`; no Wasm plugin is required.
